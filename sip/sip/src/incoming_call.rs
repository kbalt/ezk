...
use rtc::state::sdp::{ParseSessionDescriptionError, SessionDescription};
...
impl IncomingCall<NoMedia> {
    ...
    pub fn from_invite(
        ...
        contact: Contact,
    ) -> Result<Self, (IncomingRequest, IncomingCallFromInviteError)> {
            ...
            .is_ok_and(|content_type| content_type == CONTENT_TYPE_SDP)
        {
            let utf8_body = match BytesStr::from_utf8_bytes(invite.body.clone()) {
                ...
                Err(e) => return Err((invite, IncomingCallFromInviteError::InvalidUtf8Body(e))),
            };

            let sdp_offer = match SessionDescription::parse(&utf8_body) {
                Ok(sdp_offer) => sdp_offer,
                Err(e) => {
                    return Err((invite, IncomingCallFromInviteError::InvalidSDP(e)));
                }
            };
        ...
        let dialog = match Dialog::new_server(endpoint.clone(), &invite, contact) {
            Ok(dialog) => dialog,
            Err(e) => return Err((invite, IncomingCallFromInviteError::CreateDialog(e))),
        };
