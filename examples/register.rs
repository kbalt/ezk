...
async fn main() -> Result<()> {
    ...
    let user_agent = UserAgent::builder()
        .with_udp_transport("0.0.0.0:5070")
        .await?
        .build()
        .await;

    // Build account config and add some credentials
    let mut config = AccountConfig::new("3270001e0".into(), SipUri::from_str("sip:sipgate.de")?);
    config.auth.credentials.add_for_realm(
        "sipgate.de",
        DigestCredentials::new("3270001e0", "sc72WMotAYAg"),
    );
