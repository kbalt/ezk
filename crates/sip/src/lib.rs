...
mod tests {
    ...
    use ezk_session::{BundlePolicy, Codec, Codecs, Options, RtcpMuxPolicy, TransportType};
    ...
    async fn test() {
        ...
        let mut sdp_session = ezk_session::AsyncSdpSession::new(
        ...
        let audio = sdp_session
            .add_local_media(
                Codecs::new(ezk_session::MediaType::Audio).with_codec(Codec::G722),
                1,
                ezk_session::Direction::SendRecv,
            )
            .unwrap();

        let video = sdp_session
            .add_local_media(
                Codecs::new(ezk_session::MediaType::Video).with_codec(Codec::H264.with_pt(97)),
                1,
                ezk_session::Direction::SendRecv,
            )
            .unwrap();

        sdp_session.add_media(audio, ezk_session::Direction::SendRecv);
        sdp_session.add_media(video, ezk_session::Direction::SendRecv);
